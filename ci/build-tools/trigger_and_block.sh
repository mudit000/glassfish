  ...
  do
    ...
    if ${completed} ; then
      ...
      for ((i=0;i<${#triggers[@]};i++))
      {
        ...
        local job_build_number=${trigger_entry[1]}
        local job_url=${TEST_JOB_URL}/${job_build_number}
        local job_attempt_count=${trigger_retry_count[${i}]}
        # job is alredy known to be aborted
        if [ ${trigger_status[${i}]} -eq 1 ]; then
          add_to_test_status "${test_id}" "${job_url}" "ABORTED" "${job_attempt_count}"
        else
          # job is complete
          ...
          do
            ...
            if [ ${error_code} -eq 0 ] ; then
              add_to_test_status "${test_id}" "${job_url}" "${test_status}" "${job_attempt_count}"
              break
            fi
    ...
    if [ ${count} -gt 420 ] ; then
      for ((i=0;i<${#triggers[@]};i++))
      {
        ...
        local job_build_number=${trigger_entry[1]}
        local job_url=${TEST_JOB_URL}/${job_build_number}
        local total_attempt=${trigger_retry_count[${i}]}
        if [ ${trigger_status[${i}]} -eq 2 ]; then
          local test_status=`get_test_job_status ${job_build_number}`
          add_to_test_status "${test_id}" "${job_url}" "${test_status}" "${total_attempt}"
        else
          add_to_test_status "${test_id}" "${job_url}" "ABORTED" "${total_attempt}"
        fi
      }
