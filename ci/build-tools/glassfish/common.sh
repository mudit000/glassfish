...
EOF
    ...
    scp -o "StrictHostKeyChecking no" /tmp/purgeNightlies.sh ${SSH_MASTER}:/tmp
...
promote_init(){
    ...
    export ARCHIVE_STORAGE_BUNDLES=/tmp/${ARCHIVE_MASTER_BUNDLES}
    export SSH_MASTER=${RE_USER}@${STORAGE_HOST}
    export SSH_STORAGE=${RE_USER}@${STORAGE_HOST}
    export SCP=${SSH_STORAGE}:${ARCHIVE_STORAGE_BUNDLES}
    export ARCHIVE_URL=http://${STORAGE_HOST_HTTP}/tmp/${ARCHIVE_MASTER_BUNDLES}
    init_storage_area
}
...
#    create_symlinks
    add_permission
    create_index /content/quarantine/www/${ARCHIVE_PATH}
    send_notification
    delete_storage_area
}
...
promote_weekly(){
    promote_init "weekly"
    promote_bundle web.zip ${PRODUCT_GF}-${PRODUCT_VERSION_GF}-web-${BUILD_ID}.zip
    promote_bundle glassfish.zip ${PRODUCT_GF}-${PRODUCT_VERSION_GF}-${BUILD_ID}.zip
    promote_bundle nucleus-new.zip nucleus-${PRODUCT_VERSION_GF}-${BUILD_ID}.zip
    promote_bundle version-info.txt version-info-${PRODUCT_VERSION_GF}-${BUILD_ID}.txt
    if [ -z ${RELEASE_VERSION} ]
    then
        promote_bundle changes.txt changes-${PRODUCT_VERSION_GF}-${BUILD_ID}.txt
    ...
    then    
        ...
        echo $NEXT_BUILD_ID > ${HUDSON_HOME}/promote-trunk.version
        echo $NEXT_ID > ${HUDSON_HOME}/pkgid-trunk.version
...
#    purge_old_nightlies
    ...
    curl -H "Content-Type: application/json" \
        ...
        -X POST \
        -k \
        https://registry.hub.docker.com/u/oracle/glassfish/trigger/9931b24b-54cf-4783-a9dc-78cada48fc1b/  
    #ssh $SSH_MASTER `echo "echo $SCM_REVISION > ${HUDSON_HOME}/last_promoted_nightly_scm_revision"`
    #echo $SCM_REVISION > ${HUDSON_HOME}/last_promoted_nightly_scm_revision
    promote_finalize
}

init_common(){
    require_env_var "HUDSON_HOME"
    #BUILD_ID="b01" #cat ${HUDSON_HOME}/promote-trunk.version`
    #PKG_ID="b01" #cat ${HUDSON_HOME}/pkgid-trunk.version`
...
release_prepare(){
    printf "\n%s \n\n" "===== UPDATE POMS ====="
    mvn ${MAVEN_ARGS} -f pom.xml release:prepare \
...
zip_tests_maven_repo(){
    ...
    zip -r ${WORKSPACE}/bundles/tests-maven-repo.zip \
        ...
        org/glassfish/hk2/glassfish-nucleus-hk2/* \
        org/glassfish/hk2/config-generator/* > /dev/null
        ...
        #org/glassfish/hk2/config-types/* \
        #org/glassfish/hk2/glassfish-nucleus-hk2/* \
        #org/glassfish/hk2/config-generator/* \        
...
cp -rf /scratch/BUILD_AREA/bundles/* ${NFS_PATH}/bundles 
...
EOF
    ...
    scp -o "StrictHostKeyChecking no" ${PROMOTE_SCRIPT} ${SSH_MASTER}:/tmp
...
scp_jnet(){
    ...
    ssh -o "StrictHostKeyChecking no" ${SSH_MASTER} \
        "scp ${ARCHIVE_STORAGE_BUNDLES}/${file} ${JNET_DIR}"
    ssh -o "StrictHostKeyChecking no" ${SSH_MASTER} \
        "scp ${ARCHIVE_STORAGE_BUNDLES}/${file} ${JNET_DIR}/latest-${simple_name}"
}
...
create_promotion_changs(){
    ...
    if [[ "nightly" == "${BUILD_KIND}" ]]; then
        ...
        PREVIOUS_COMMIT=`head -1 ${LAST_STABLE_PROM_SCM_VERSION_FILE} | cut -d ":" -f2 | tr -d ' '`
    fi
...
add_permission(){
    ssh ${SSH_MASTER} ssh ${JNET_USER}@${JNET_STORAGE_HOST} exec /bin/bash -s << EOF
    cd /content/quarantine/www/${ARCHIVE_PATH}
    find . -type f ! -name \*.UVSCAN_IN_PROGRESS  -exec chmod o+r {} +
EOF
}
...
#
create_index(){
    sync_complete=$(ssh ${SSH_MASTER} ssh ${JNET_USER}@${JNET_STORAGE_HOST} exec /bin/bash -s << EOF 
        sync_complete=false
        cd ${1}
        for (( i = 0; i < 400; i++ )); do
...
EOF
    ...
    if [ "$sync_complete" != true ] ; then
        echo 'sync job between /content/quarantine/www and /dlc is not working'
        exit 1
    fi
    # cp this script to master.
    scp -o "StrictHostKeyChecking no" /gf-hudson-tools/${hudson_tools}/build-tools/glassfish/common.sh ${SSH_MASTER}:/tmp

    # cp script from ssh_master to JNET_STORAGE_HOST.
    ssh -o "StrictHostKeyChecking no" ${SSH_MASTER} \
        "scp /tmp/common.sh ${JNET_USER}@${JNET_STORAGE_HOST}:/content/quarantine/www/${PRODUCT_GF}"
   
    # Run the script to create the index files on JNET_STORAGE_HOST.
    ssh ${SSH_MASTER} \
        "ssh ${JNET_USER}@${JNET_STORAGE_HOST} 'cd /content/quarantine/www/${PRODUCT_GF}; bash -c \"source common.sh; generate_index_html /dlc/${PRODUCT_GF}/ | tee /tmp/gen.out 2>&1\"'"
}
...
#
trim_dot_slash() {
    printf "\nInside trim_dot_slash: called with $1\n" 
  # If FILE starts with with "./" then remove it.
  if [[ $1 == ./* ]]; then
    FILE=`echo $1 | cut -c 16-`
  fi
}
...
cat >> $1/$INDEX_FILENAME << EOF
        <tr class="parent">
          ...
          <td>
            <a href="$TOPLEVELURL/`echo $1 | cut -c 16-`/../$INDEX_FILENAME">Parent Directory</a></td><td>&nbsp;
          </td>
          <td align="right">  - </td>
        </tr>
EOF
  ...
  do
    ...
    if [ "$SUB_DIR" != "." ] && [ "$SUB_DIR" != "$1" ] && [ "$SUB_DIR" != "/dlc/glassfish" ] && [ -d "$SUB_DIR" ] && [[ $SUB_DIR_BASENAME != .* ]] && [ "$SUB_DIR_BASENAME" != "listing_themes" ]
    then
      ...
      then
        ...
        HREF="$TOPLEVELURL/`echo $1 | cut -c 16-`/$SUB_DIR_BASENAME/$INDEX_FILENAME"
      fi 
...
cat >> $1/$INDEX_FILENAME << EOF
        <tr>
          <td valign="top">
            <a href="$TOPLEVELURL/`echo $FILE | cut -c 16-`"><img src="$TOPLEVELURL/listing_themes/apaxy/$ICON" alt="[   ]"></a>
          </td>
          <td>
            <a href="$TOPLEVELURL/`echo $FILE | cut -c 16-`">$FILENAME</a>
          </td>
